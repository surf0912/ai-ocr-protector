...
@router.get("/")
def list_novels(user: dict = Depends(get_current_user), sb: Client = Depends(get_supabase)):
    if user["role"] == "admin":
...
@router.get("/{novel_id}")
def get_novel(novel_id: str, user: dict = Depends(get_current_user), sb: Client = Depends(get_supabase)):
    _check_novel_access(novel_id, user, sb)
    res = sb.table("novels").select("*").eq("id", novel_id).single().execute()
    return res.data

@router.post("/", dependencies=[Depends(require_admin)])
def create_novel(body: NovelCreate, user: dict = Depends(require_admin), sb: Client = Depends(get_supabase)):
    res = sb.table("novels").insert({**body.dict(), "created_by": user["id"]}).execute()
    return res.data[0]

@router.patch("/{novel_id}", dependencies=[Depends(require_admin)])
def update_novel(novel_id: str, body: NovelUpdate, sb: Client = Depends(get_supabase)):
...
def delete_novel(novel_id: str, sb: Client = Depends(get_supabase)):
