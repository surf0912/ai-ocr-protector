...
@router.get("/my")
def my_permissions(user: dict = Depends(get_current_user), sb: Client = Depends(get_supabase)):
    res = (
...
def novel_permissions(novel_id: str, sb: Client = Depends(get_supabase)):
    res = (
        sb.table("permissions")
...
def grant(body: GrantRequest, user: dict = Depends(require_admin), sb: Client = Depends(get_supabase)):
...
def revoke(body: GrantRequest, user: dict = Depends(require_admin), sb: Client = Depends(get_supabase)):
